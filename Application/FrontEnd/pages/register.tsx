...
// Simple registration page. Attempts to register and then log the user in
// automatically so they don't need to re-enter credentials.
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.user) {
        ...
        try {
          const l = await fetcher('/auth/login', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ username, password }),
          });
          ...
          // ignore login error and fall back to showing login page
        }
        router.push('/login');
      } else {
